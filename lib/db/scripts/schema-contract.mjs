...
function numericShape(type) {
  const match = type.match(/^(?:numeric|decimal)\((\d+),\s*(\d+)\)$/);
  return match
...
export function potentiallyDestructiveTypeChange(expected, actual) {
  ...
  const expectedNumeric = numericShape(expected);
  const actualNumeric = numericShape(actual);
  if (expectedNumeric && actualNumeric) {
    return (
      actualNumeric.precision < expectedNumeric.precision ||
      actualNumeric.scale < expectedNumeric.scale
    );
  }
  const integerRank = new Map([
    ["smallint", 1],
    ["integer", 2],
    ["bigint", 3],
  ]);
  return (
    integerRank.has(expected) &&
    integerRank.has(actual) &&
    integerRank.get(actual) < integerRank.get(expected)
  );
}
