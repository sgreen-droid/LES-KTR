...
test("classifies numeric and integer narrowing as destructive", () => {
  assert.equal(
    potentiallyDestructiveTypeChange("numeric(12, 4)", "numeric(10, 2)"),
    true,
  );
  assert.equal(potentiallyDestructiveTypeChange("bigint", "integer"), true);
  assert.equal(potentiallyDestructiveTypeChange("integer", "bigint"), false);
});
